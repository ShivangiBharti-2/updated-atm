...
public class SignUpOne extends JFrame implements ActionListener {
    ...
    {   setLayout(null);
        ...
        Random ran = new Random();                                                /*Random : class present in util package used for random numbers*/
        random = (Math.abs((ran.nextLong() % 9000L) + 1000L));                    /*Math.abs - convert random number to positive number*/
        ...
        add(married);  
        unmarried = new JRadioButton("Unmarreid");
    ...
    {
       String formno = "" + random;                                     /*random is of long type but we are converting long into string by using ("" +)*/
       String name = nameTextField.getText();                           /*getText() : to get value from textfield*/
       ...
       try
       {
          if (name.equals("")) {                                                                  /*if name is blank*/
              JOptionPane.showMessageDialog(null,"Name is required");                  /*JOptionPane to show popup*/
          }else {
              Connect c = new Connect();                                                                 /*creating connection with database and insert values in signup table*/
              String query = "insert into   signup values ('"+formno+"','"+name+"','"+fname+"','"+dob+"','"+gender+"','"+email+"','"+marital+"','"+address+"','"+city+"','"+state+"','"+pincode+"')";
              c.s.executeUpdate(query);
              setVisible(false);                                                                    /*if user fiiled the form athrn we have to close the current frame*/
              new SignUpTwo(formno).setVisible(true);                                                /*object of class signuptwo*/
          }
       }catch(Exception e)
       {
